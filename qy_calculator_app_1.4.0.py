...
# -*- coding: utf-8 -*-
# qy_calculator_app 1.4.0
# Created by Vince on 2025/03/27
...
class QYCalculatorApp(TkinterDnD.Tk):
    ...
    def setup_right_panel(self, parent):
        """右侧面板初始化"""
        self.right_panel = ttk.LabelFrame(parent, text="相对量子产率计算", width=750)
    ...
    def _setup_solvent_tab(self, parent):
        ...
        # 绑定右键菜单
        self.solvent_menu = tk.Menu(self, tearoff=0)
        self.solvent_menu.add_command(label="复制", command=lambda: self.copy_refractive_index(tree))
    
        def on_right_click(event):
            item = tree.identify_row(event.y)
            if item:
                tree.selection_set(item)
                self.solvent_menu.post(event.x_root, event.y_root)
    ...
    def setup_history_panel(self):
        ...
        # self.history_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, pady=0)
    ...
    def setup_context_menu(self):
        ...
        self.history_menu.add_command(label="修改名称", command=self.rename_sample,font=('微软雅黑'))
        self.history_menu.add_command(label="删除记录", command=self.delete_record,font=('微软雅黑'))
        self.history_menu.add_separator()
        self.history_menu.add_command(label="清空历史", command=self.clear_history,font=('微软雅黑'))
        self.history_menu.add_command(label="导出Excel", command=self.export_history,font=('微软雅黑'))
    ...
    def clear_file(self):
        ...
        if hasattr(self, 'current_file_type'):
            self.file_data[self.current_file_type] = None
            getattr(self, f"{self.current_file_type}_file_label").config(text="未选择文件")
            self.status_label.config(text=f"{'标液' if self.current_file_type=='std' else '样品'}文件已清除")
            if self.current_file_type == "std":
    ...
    def calculate_integral(self, file_type):
        ...
        try:
            ...
            # 获取对应参数
            if file_type == "std":
                method = self.std_method.get()
                start = float(self.std_start_wl.get())
                end = float(self.std_end_wl.get())
                target_entry = 'i_ref'
            else:
                method = self.sample_method.get()
                start = float(self.sample_start_wl.get())
                end = float(self.sample_end_wl.get())
                target_entry = 'i_sample'
            ...
            self.status_label.config(text=f"{'标液' if file_type=='std' else '样品'}积分计算完成")
                    # 显示积分结果（第3处修改）
            if file_type == "std":
                self.std_integral_label.config(
                    text=f"积分结果：{integral:.4f}",
                    foreground="#27ae60"  # 成功时绿色
                    )
            else:
                self.sample_integral_label.config(
                    text=f"积分结果：{integral:.4f}",
                    foreground="#27ae60")
